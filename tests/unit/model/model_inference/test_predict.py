...
def test_predict_function_with_mocker(mocker):
    ...
    mock_load_pipeline = mocker.patch(
        "src.model.model_inference.load_pipeline", return_value=mock_pipeline
    )

    # Call the predict function using the module reference
    prediction_result = model_inference.predict(
        sample_data, model_path="dummy/path.joblib"
    )
